...
class WebSearchManager:
    ...
    def search(
        ...
        if provider == "tavily" and self.tavily_tools:
            tried_providers.append("tavily")
            results = self._search_tavily(query, max_results, search_type)
            
            # If Tavily fails, automatically try Exa
            if not results.get("ok") and self.exa_tools:
                print(f"⚠️  Tavily search failed, trying Exa as fallback...")
                tried_providers.append("exa")
            ...
            # If Exa fails, automatically try Tavily
            if not results.get("ok") and self.tavily_tools:
                print(f"⚠️  Exa search failed, trying Tavily as fallback...")
                tried_providers.append("tavily")
                results = self._search_tavily(query, max_results, search_type)
        ...
        if not results or not results.get("ok"):
            return {
                ...
                "message": f"Search failed after trying: {', '.join(tried_providers)}",
                "tried_providers": tried_providers,
            }
    ...
    def search_error_solution(
        ...
        return self.search(
            query=query,
            provider="tavily",  # Tavily better for error solutions, will fallback to Exa if fails
            max_results=max_results,
            search_type="general",
        )
