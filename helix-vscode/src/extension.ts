...
import { AgentOSClient, WorkspaceContext } from './api/agentosClient';
import * as tools from './tools/workspaceTools';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('helix.startServer', startServer),
        vscode.commands.registerCommand('helix.stopServer', stopServer),
        vscode.commands.registerCommand('helix.showPreview', showPreview),
        vscode.commands.registerCommand('helix.runWorkflow', runWorkflow),
        vscode.commands.registerCommand('helix.selectAgent', selectAgent)
    );
...
async function handleChatRequest(
    ...
    try {
        ...
        const response = await agentOSClient.chat(agentId, request.prompt, {
            ...
            workspace: workspaceContext as WorkspaceContext
        });
...
async function executeTool(
    ...
    switch (tool) {
        ...
        case 'editFile':
        case 'edit_file':
            result = await tools.editFile(
                parameters.path,
                parameters.old_content || parameters.oldContent,
                parameters.new_content || parameters.newContent
            );
            if (result.success) {
                stream.button({
                    command: 'vscode.open',
    ...
    try {
        ...
        if (isHealthy) {
            ...
            const action = await vscode.window.showWarningMessage(
                '⚠️ Helix AgentOS is not responding',
                'Start Server'
            );
...
function generateSessionId(chatContext: vscode.ChatContext): string {
